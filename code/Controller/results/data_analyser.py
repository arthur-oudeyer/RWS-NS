...
class SimpleVideoPlayer(tk.Label):
    """
    Looping video player built on av + Pillow.
    Preloads all frames into memory (fine for short rollout clips), then plays
    them via tkinter after() — no shared mutable state between threads.
    """
    ...
    def __init__(self, master, **kwargs):
        ...
        self._frames: list = []
        self._delay_ms: int = 40
        self._idx: int = 0
        self._playing: bool = False
        self._after_id = None
        self._imgtk = None
        self._gen: int = 0   # incremented on stop() to cancel stale callbacks
    ...
    def play(self):
        ...
        if not self._frames:
            gen = self._gen
            threading.Thread(target=self._preload, args=(gen,), daemon=True).start()
        else:
            self._idx = 0
    ...
    def _preload(self, gen: int):
        ...
        try:
            self.after(0, lambda: self._start(gen))
        except Exception:
            pass

    def _start(self, gen: int):
    ...
    def _tick(self, gen: int):
        if gen != self._gen or not self._playing or not self._frames:
            return
        ...
        self.config(image=self._imgtk)
        self._idx = (self._idx + 1) % len(self._frames)
        self._after_id = self.after(self._delay_ms, lambda: self._tick(gen))
...
def _find_video(run_dir: Path, individual: dict) -> Optional[Path]:
    ...
    if vp:
        ...
        if p.exists():
            return p
        # video_path is relative to controller root (run_dir.parent.parent)
        p2 = run_dir.parent.parent / vp
        if p2.exists():
            return p2

    gen = individual.get("generation", 0)
    iid = individual.get("individual_id", 0)
    candidate = run_dir / "videos" / f"gen{gen:04d}_id{iid:06d}.mp4"
    if candidate.exists():
        return candidate

    videos_dir = run_dir / "videos"
    if videos_dir.exists():
        for found in videos_dir.rglob(f"*id{iid:06d}.mp4"):
            return found
...
def draw_reward(ax, run_data: dict, sel_id) -> None:
    if sel_id is None:
        return _no_data(ax, "Select an individual")
    all_ind = run_data.get("all_individuals", {})
    indiv   = all_ind.get(sel_id)
    ...
    desc = {k: v for k, v in (indiv.get("reward_weights") or {}).items()
            if isinstance(v, (int, float))}
    if not desc:
        return _no_data(ax, "No numeric reward_weights available")
    ...
    ax.set_yticklabels([k.replace("_", " ") for k in keys], fontsize=8)
    ax.axvline(0, color="black", lw=0.8)
    ax.set_title(f"Reward Weights  —  id={sel_id}")
    ax.set_xlabel("Value")
    ax.grid(True, axis="x", alpha=0.3)
...
GRAPH_RENDERERS: dict = {
    ...
    "Individual Reward":                draw_reward,
    "Map-Elites Grid Coverage":         draw_grid_coverage,
}

# graphs that need a refresh when the selected individual changes
_INDIVIDUAL_GRAPHS = {"Genealogy Path", "Individual Render", "Individual Descriptors", "Individual Reward",
...
class DataAnalyser:
    ...
    def _build_ui(self):
        ...
        # Get screen height and set a maximum height for the window
        screen_height = self.root.winfo_screenheight()
        max_window_height = int(screen_height * 0.9)  # Use 90% of screen height

        pw = tk.PanedWindow(self.root, orient=tk.HORIZONTAL,
                             sashwidth=6, sashrelief=tk.RAISED, bg="#CCCCCC", height=max_window_height)
    ...
    def _build_graphs(self, parent: tk.Frame):
        vpw = tk.PanedWindow(parent, orient=tk.VERTICAL,
                             sashwidth=5, sashrelief=tk.RAISED, bg="#AAAAAA")
        vpw.pack(fill=tk.BOTH, expand=True)
        self._vpw = vpw

        # ── Graph 1 ──
        g1 = tk.Frame(vpw, bg="#E8E8E8")
        vpw.add(g1, minsize=180)
        ...
        self._vid1_frame, self._vid1_player, self._vid1_info_lbl = \
            self._build_video_panel(g1, 1)

        # ── Graph 2 ──
        g2 = tk.Frame(vpw, bg="#E8E8E8")
        vpw.add(g2, minsize=180)
        ...
        self._vid2_frame, self._vid2_player, self._vid2_info_lbl = \
            self._build_video_panel(g2, 2)

    def _build_video_panel(self, parent: tk.Frame, n: int):
        """Build a hidden video-player frame inside *parent*; return (frame, player, info_lbl)."""
        frame = tk.Frame(parent, bg="black")
        # Not packed yet — toggled by _refresh_graph

        player = SimpleVideoPlayer(frame)
        player.pack(fill=tk.BOTH, expand=True)

        ctrl_bar = tk.Frame(frame, bg="#222222")
        ctrl_bar.pack(fill=tk.X, side=tk.BOTTOM)

        info_lbl = tk.Label(ctrl_bar, text="", bg="#222222", fg="#DDDDDD",
                            font=("Courier", 10), anchor=tk.W)
        info_lbl.pack(side=tk.LEFT, padx=6, fill=tk.X, expand=True)

        for symbol, cmd in [("▶", "play"), ("⏸", "pause"), ("⏹", "stop")]:
            tk.Button(ctrl_bar, text=symbol, width=3,
                      command=lambda c=cmd, _n=n: self._vid_cmd(_n, c)
                      ).pack(side=tk.RIGHT, padx=2, pady=2)

        return frame, player, info_lbl
    ...
    def _build_info(self, parent: tk.Frame):
        ...
        self._run_id_lbl.pack(fill=tk.X, padx=2, pady=(2, 4))

        # ── Controls (blue) — packed at bottom first so they are always visible ──
        ctrl = tk.Frame(parent, bg=COLOR_CTRL_BG, relief=tk.GROOVE, bd=2)
        ctrl.pack(fill=tk.X, side=tk.BOTTOM, padx=2, pady=(0, 4))
        ...
        # ── Text sections — fill remaining space, each with internal scrollbar ──
        text_area = tk.Frame(parent, bg=COLOR_INFO_BG)
        text_area.pack(fill=tk.BOTH, expand=True, padx=2)

        # General info
        tk.Label(text_area, text="General info :", bg=COLOR_INFO_BG,
                 font=FONT_BOLD, anchor=tk.W, padx=6).pack(fill=tk.X)
        gf = tk.Frame(text_area, bg=COLOR_INFO_BG)
        gf.pack(fill=tk.BOTH, expand=True, padx=4, pady=(0, 4))
        ...
        self._gen_txt = tk.Text(gf, font=FONT_MONO, bg=COLOR_INFO_BG, fg="black",
                                ...
                                height=8, yscrollcommand=g_sb.set)
        self._gen_txt.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        g_sb.config(command=self._gen_txt.yview)

        # Individual info
        tk.Label(text_area, text="Individual selected info :", bg=COLOR_INFO_BG,
                 font=FONT_BOLD, anchor=tk.W, padx=6).pack(fill=tk.X)
        if_ = tk.Frame(text_area, bg=COLOR_INFO_BG)
        if_.pack(fill=tk.BOTH, expand=True, padx=4, pady=(0, 4))
        ...
        self._ind_txt = tk.Text(if_, font=FONT_MONO, bg=COLOR_INFO_BG, fg="black",
                                relief=tk.FLAT, state=tk.DISABLED, wrap=tk.WORD,
                                height=8, yscrollcommand=i_sb.set)
    ...
    def _refresh_graph(self, n: int):
        ...
        if n == 1:
            ...
            vid_frame = self._vid1_frame
        else:
            fig, cvs = self._fig2, self._cvs2
            var, hdr = self._g2_var, self._g2_hdr
            vid_frame = self._vid2_frame

        name = var.get()
        hdr.config(text=name)

        if name == "Individual Render":
            cvs.get_tk_widget().pack_forget()
            vid_frame.pack(fill=tk.BOTH, expand=True)
            self._load_video_for_panel(n)
            self.root.after(10, self._adjust_sash)
            return

        # Switching away from video: stop playback and restore matplotlib canvas
        self._stop_video(n)
    ...
    def _adjust_sash(self):
        """Reposition the vertical sash"""
        self._vpw.sash_place(0, 0, self._vpw.winfo_height() // 2)

    # ── Video player helpers ───────────────────────────────────────────────────

    def _load_video_for_panel(self, n: int):
        player   = self._vid1_player   if n == 1 else self._vid2_player
        info_lbl = self._vid1_info_lbl if n == 1 else self._vid2_info_lbl

        if not self._run_data or self._selected_id is None:
            info_lbl.config(text="No individual selected")
            return

        indiv = self._run_data["all_individuals"].get(self._selected_id)
        if indiv is None:
            info_lbl.config(text=f"Individual id={self._selected_id} not found")
            return

        video_path = _find_video(self._run_data["run_dir"], indiv)
        if video_path is None:
            info_lbl.config(text=f"No video found for id={self._selected_id}")
            return

        player.load(str(video_path))
        player.play()
        info_lbl.config(
            text=f"id={self._selected_id}  gen={indiv['generation']}  "
                 f"fit={indiv['fitness']:.4f}    {video_path.name}"
        )

    def _stop_video(self, n: int):
        player = self._vid1_player if n == 1 else self._vid2_player
        player.stop()

    def _vid_cmd(self, n: int, cmd: str):
        player = self._vid1_player if n == 1 else self._vid2_player
        if cmd == "play":
            player.play()
        elif cmd == "pause":
