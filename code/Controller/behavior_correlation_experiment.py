"""
behavior_correlation_experiment.py
===================================
Compares human behaviour evaluations to Gemini VLM scores and produces
correlation graphs — the dynamic-behaviour counterpart of
Morphology/correlation_experiment.py.

Workflow
--------
1. Load the human-annotated dataset from
       utils/behavior_human_eval_dataset/dataset.json
   (built with utils/behavior_human_evaluation.py).
2. For each behaviour, load its saved MP4 and score it with LocomotionGrader
   using the exact target behaviour the human annotator defined. Entries that
   share the same target are batched in a single API call.
3. Compute Pearson correlations (and p-values) between human and VLM scores
   on three dimensions: coherence, originality, potential, plus an overall average.
4. Save per-behaviour VLM scores and correlation stats to JSON.
5. Generate scatter plots with regression lines (requires matplotlib).

Scales
------
Human scores are 0–100 integers (sliders). The VLM grader returns each
dimension in [0, 1]; we rescale by ×100 so both live on the same 0–100 axis.
(Pearson r is scale-invariant, so this only affects the plots, not the stats.)

Usage
-----
    python behavior_correlation_experiment.py
    python behavior_correlation_experiment.py --output results/behavior_corr_01
    python behavior_correlation_experiment.py --vlm-scores results/behavior_corr_01/vlm_scores.json
    python behavior_correlation_experiment.py --api-key YOUR_GEMINI_KEY

Arguments
---------
    --api-key       Gemini API key (defaults to APIKEY_GEMINI from code/api_keys.py)
    --dataset       Path to dataset.json  (default: utils/behavior_human_eval_dataset/dataset.json)
    --output        Output directory       (default: results/behavior_correlation)
    --model         Gemini model ID        (default: gemini-3-flash-preview)
    --batch-size    Videos per API call    (default: 6)
    --vlm-scores    Path to a pre-computed vlm_scores.json to skip re-scoring
    --debug         Verbose grader output

Output in <output>/
-------------------
    vlm_scores.json          — per-behaviour VLM scores + reasoning
    correlation_results.json — Pearson r, p-value and n per dimension
    plot_coherence.png / plot_originality.png / plot_potential.png / plot_overall.png
...
from gemini_prompts import LocomotionPromptConfig, LocomotionScoringWeights, build_locomotion_prompt
...
# ── constants ────────────────────────────────────────────────────────────────
_DATASET_DIR  = _SCRIPT_DIR / "utils" / "behavior_human_eval_dataset"
_DATASET_FILE = _DATASET_DIR / "dataset.json"
DIMENSIONS    = ("coherence", "originality", "potential")
_VLM_SCALE    = 100.0   # grader returns [0,1]; rescale to the human 0–100 axis
...
def load_dataset(path: Path) -> list[dict]:
    if not path.exists():
        ...
        print("Run utils/behavior_human_evaluation.py first to build the dataset.")
        sys.exit(1)
    with open(path) as f:
        data = json.load(f)
    if not data:
        print("ERROR: Dataset is empty — annotate some behaviours first.")
        sys.exit(1)
    print(f"Loaded {len(data)} annotated behaviours from {path}")
    return data
...
def run_vlm_evaluation(
    ...
    """
    Score each behaviour's MP4 with LocomotionGrader using the human-defined
    target. Entries are grouped by target so each unique target shares one
    grader instance and one batch API call.

    Returns
    -------
    dict[behavior_id] -> {coherence, originality, potential, fitness,
                          observation, interpretation, *_reason}
    """
    ...
    for target, entries in groups.items():
        print(f"\n[VLM] {len(entries)} behaviours  [target='{target}']")

        prompt_config = LocomotionPromptConfig(
            name    = f"corr_{target.replace(' ', '_')[:20]}",
            target  = target,
            prompt  = build_locomotion_prompt(target),
        ...
        for entry in entries:
            vid_path = _DATASET_DIR / entry["video_path"]
            if not vid_path.exists():
                print(f"  WARNING: video not found: {vid_path} — skipping {entry['id']}")
                continue
            videos.append((entry["id"], str(vid_path)))

        if not videos:
            print("  No videos found for this group — skipping.")
            continue

        try:
            batch_out = grader.score_batch(videos, debug=debug)
        except Exception as exc:
            print(f"  ERROR during batch scoring: {exc}")
            continue

        for bid, output in batch_out.items():
            results[bid] = {
                ...
                "potential_reason":   output.extra.get("potential_reason",   ""),
            }
            if debug:
                v = results[bid]
                print(f"  {bid}: coh={v['coherence']:.2f}  "
...
def compute_correlations(
    ...
    """
    Pair human (0–100) and VLM (rescaled to 0–100) scores and compute Pearson
    correlations per dimension plus an overall average.
    ...
    for entry in dataset:
        ...
        for dim in DIMENSIONS:
            ...
            paired[dim]["vlm"].append(float(v[dim]) * _VLM_SCALE)

        paired["overall"]["human"].append(sum(h[d] for d in DIMENSIONS) / len(DIMENSIONS))
        paired["overall"]["vlm"].append(
            sum(v[d] * _VLM_SCALE for d in DIMENSIONS) / len(DIMENSIONS))
...
_REGLINE  = "#ff8844"
_DIM_COLOR = {
    "coherence": "#88aaff", "originality": "#ffaa44",
    "potential": "#88ff88", "overall": "#cccccc",
}
...
def _make_axes(ax):
    ...
    ax.set_xlabel("Human score (0–100)", color="#aaaaaa", fontsize=9)
    ax.set_ylabel("VLM score (0–100)",   color="#aaaaaa", fontsize=9)
...
def plot_all(correlations: dict, output: Path) -> None:
    ...
    fig.suptitle(
        "Human vs VLM Behaviour Evaluation — Correlation",
...
def main():
    parser = argparse.ArgumentParser(
        description="Correlation experiment: human vs VLM behaviour evaluation"
    )
    ...
    parser.add_argument("--batch-size", type=int, default=6,
                        help="Videos per batch API call")
    ...
    if args.vlm_scores:
        ...
        print(f"\nScored {len(vlm_scores)} / {len(dataset)} behaviours.")
