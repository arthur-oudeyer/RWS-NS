...
ROBOT = RobotMorphology(
    ...
    legs  = [
        ...
        LegDescriptor(placement_angle_deg=0.0,
                      joints=[JointDescriptor(rgba=(0.4, 0.8, 0.5, 1.0), length=0.1, rest_angle=1.57, ctrl_range=(-3.14, 3.14))],
                      parent_leg_idx=3, parent_joint_idx=0),
    ],
    torso_a=0.08,
    torso_b=0.08,
...
PRECONFIGURED = (QUADRIPOD, TRIPOD, HEXAPOD, HUMAN, ROBOT)

def get_preconfigured_morph(name: str):
    for mo in PRECONFIGURED:
        if mo.name == name:
            return mo
