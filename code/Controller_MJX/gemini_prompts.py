...
def build_locomotion_prompt(target_behaviour: str) -> str:
    ...
    You are a strict and skeptical evaluator analysing a short physics-simulation
    video of a robot in MuJoCo. Your job is to be PRECISE and CONSERVATIVE — do
    not give benefit of the doubt and do not assume movement if you are unsure.

    The scene of each video is composed of :
    - 2 horizontally merged views of the same scene. The left view is a side-view of the robot, the right view is a 3/4 view.
    - Each view has a fixed-pose camera.
    - The floor is a grassy green checkerboard, with a grey-green square plate on it which is the start location of the robot; it is static and used as reference for spatial distances.
    - The robot has white ellipsoidal body parts (like the base torso) and colored legs with yellow spheres as feet.
    ...
    Be specific. Note the moment of any event, the position of the limbs, the displacement of the robot between frames. Use the starting plate as relative displacement / movement reference; if the robot is not on it, it has moved.
    ...
    interest — Is the gait pattern interesting, biologically plausible and leading to real evolutionary potential ?
...
def generate_fake_vlm_batch_response(robot_ids) -> str:
    ...
    for robot_id in robot_ids:
        robots_data[robot_id] = {
            ...
            "interest":       {"score": random.randint(0, 100), "reason": f"Interest reason for {robot_id}."},
        }
...
if __name__ == "__main__":
    for c in ALL_LOCOMOTION_PROMPT_CONFIGS.values():
        ...
        print(f"  weights : coherence={c.weights.coherence} "
              f"originality={c.weights.originality} interest={c.weights.interest}")
        print()
